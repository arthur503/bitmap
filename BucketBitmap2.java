...
public class BucketBitmap2 {
	
	
	
	public static void main(String[] argv){
		
		BucketBitmap2 bbt = new BucketBitmap2(100);
	 ...
	 * @param capacity
	 */
	public BucketBitmap2(int capacity){
		this.capacity = capacity;
		this.bitmap = new int[getIntIndex(capacity-1)+1];		//position is between [0,capacity-1].
//		System.out.println("capacity:"+capacity+"\tbitmap length:"+bitmap.length);
	...
	public void computeBitmapData(int position){
		int start_bit_index = getBitIndex(position);
		int start_int_index = getIntIndex(position);
		int end_bit_index;
		int end_int_index;
		if((start_bit_index + (BITS_PER_BUCKET-1)) <= (BITS_PER_INT-1)){
			end_bit_index =  start_bit_index + (BITS_PER_BUCKET-1);
			end_int_index = start_int_index;
		}else{
			end_bit_index = (start_bit_index + (BITS_PER_BUCKET-1)) % BITS_PER_INT;
			end_int_index = start_int_index + (start_bit_index + (BITS_PER_BUCKET-1)) / BITS_PER_INT;
		}
		...
		if(start_int_index == end_int_index){
